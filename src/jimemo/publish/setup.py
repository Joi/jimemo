...
TOKEN_MISSING_MESSAGE = (
    ...
    "\n"
    "Then export it in your shell -- wrangler and curl read it directly "
    "from the\n"
    "environment; jimemo never stores it:\n"
    "\n"
...
def _print_intro(io: SetupIO) -> None:
    io.print(
        ...
        "shell -- setup and\n"
        "     every deploy need it exported (wrangler's own credential "
        "store is not\n"
        "     enough for the fail-closed check); jimemo never stores it. "
        "Only non-secret\n"
        "     ids (project name, account id, KV namespace id, base URL) "
        "get written to\n"
        "     ~/.jimemo/config.toml.\n"
